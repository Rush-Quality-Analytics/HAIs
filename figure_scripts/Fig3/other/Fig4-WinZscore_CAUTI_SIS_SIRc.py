...
for q in quarters:
    ...
    plt.scatter(np.sqrt(tdf1['Days']), tdf1['Winzorized z ' + metric], facecolors=tdf1['colors'], edgecolors = tdf1['edge_colors'], s=sz+20, lw=lw+0.5,
        label = 'CAUTI cases < random')
    tdf2 = tdf2.sample(n=1)
    plt.scatter(np.sqrt(tdf2['Days']), tdf2['Winzorized z ' + metric], facecolors=tdf2['colors'], edgecolors = tdf2['edge_colors'], s=sz+20, lw=lw+0.5,
        label = 'CAUTI cases ≥ random  ')
    ...
    plt.ylabel(metric + ', Winsorized z', fontsize=fs+2)
    plt.xlabel(r'$\sqrt{Urinary\ Catheter\ Days}$', fontsize=fs)
    plt.tick_params(axis='both', labelsize=fs-2)
    plt.legend(bbox_to_anchor=(-0.04, 1.02, 2.48, .2), loc=10, ncol=2, mode="expand",prop={'size':fs})
    ...
    if p75_min == p75_max:
        ...
        plt.fill_between([min(x), max(x)], p75_min, p75_max, color='0.2', linewidth=0, alpha=a)
    plt.ylabel('SIS$_\mathbf{\mathrm{C}}$' + ', Winsorized z', fontsize=fs+2)
    plt.xlabel(r'$\sqrt{Urinary\ Catheter\ Days}$', fontsize=fs)
    plt.tick_params(axis='both', labelsize=fs-2)
    ...
    plt.savefig(mydir+'/figures/Fig4_' + q + '_' + metric +'_CAUTI_SIS_SISc.png', dpi=200, bbox_inches = "tight")
    plt.close()
