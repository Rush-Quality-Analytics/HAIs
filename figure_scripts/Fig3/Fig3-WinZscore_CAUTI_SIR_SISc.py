...
for q in quarters:
    ...
    ################################## SUBPLOT 1 ############################################
    tdf = df[df['HAI'] == 'CAUTI']
    tdf['colors'] = tdf['SISc, better than random'].replace({0: '0.9', 1:'k'})
    tdf['edge_colors'] = tdf['SISc, better than random'].replace({0: ec1, 1:ec2})
    ...
    for fdate in fdates:
        ...
        p75 = np.percentile(ttdf['Winzorized z SISc'], 75)
        if p75 < p75_min:
            p75_min = float(p75)
        if p75 > p75_max:
            p75_max = float(p75)
    ...
    plt.scatter(np.sqrt(tdf1['Days']), tdf1['Winzorized z SIR'], facecolors=tdf1['colors'], edgecolors = 'k', s=sz+20, lw=lw+0.5,
        label = 'CAUTI cases < upper CL of expected')
    ...
    plt.legend(bbox_to_anchor=(-0.04, 1.02, 2.48, .2), loc=10, ncol=2, mode="expand",prop={'size':fs-2})
    ...
    if p75_min == p75_max:
        ...
        plt.fill_between([min(x), max(x)], p75_min, p75_max, color='0.2', linewidth=0, alpha=a)
    plt.ylabel('SIS, Winsorized z', fontsize=fs+2)
    plt.xlabel(r'$\sqrt{Urinary\ Catheter\ Days}$', fontsize=fs)
    plt.tick_params(axis='both', labelsize=fs-2)
    ...
    plt.savefig(mydir+'/figures/Fig3_' + q + '_' + metric +'_CAUTI_SIR_SISc.png', dpi=300, bbox_inches = "tight")
    plt.close()
