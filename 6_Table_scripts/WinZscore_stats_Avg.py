...
def agg_df(df1, fdates):
    agg_df = pd.DataFrame(columns=list(df1) + ['worst quartile SIS', 'worst quartile SIR'])
    
    for date in fdates:

        tdf = df1[df1['file date'] == date]
        p75_sis = np.percentile(tdf['Winzorized z SIS'], 75)
        p75_sir = np.percentile(tdf['Winzorized z SIR'], 75)
        
        tdf['worst quartile SIS'] = p75_sis - tdf['Winzorized z SIS']
        tdf['worst quartile SIS'] = tdf['worst quartile SIS'].apply(lambda x : 0 if x >= 0 else 1)
...
with open(mydir + "6_Table_scripts/output/WinZscore_Stats_Avg.txt", 'w+') as out:
    ...
    for hai in HAI_ls:
        ...
        n6b = [] # Avg no. of days for hospitals
        n6c = []
        n6d = []
        ...
        n17 = [] # number of hospitals with SIS of 0
        ...
        for date in fdates:
            ...
            n = tdf2[tdf2['SIS'] < 0].shape[0]
            n1.append(n)
            
            n = 100 * tdf2[tdf2['SIS'] < 0].shape[0] / tdf2.shape[0]
            n2.append(n)
            ...
            siss = tdf2['Winzorized z SIS'].tolist()
            min_sis = np.min(siss)
            n = siss.count(min_sis)
            n4.append(n)
            
            n_sir = np.nanmedian(tdf2[tdf2['Winzorized z SIR'] == min_sir]['Days'])
            n5.append(n_sir)
            
            n_sis = np.nanmedian(tdf2[tdf2['Winzorized z SIS'] == min_sis]['Days'])
            n6.append(n_sis)
            
            n = np.nanmedian(tdf2['Days'])
            n6b.append(n)
            
            pdif = 100 * np.abs(n_sir - n)/np.mean([n_sir, n])
            n6c.append(pdif)
            
            pdif = 100 * np.abs(n_sis - n)/np.mean([n_sis, n])
            n6d.append(pdif)
            
            tdf3 = tdf2[(tdf2['SIS'] < 0) & (tdf2['worst quartile SIR'] == 1)]
            n7.append(tdf3.shape[0])
            n7b.append(100 * tdf3.shape[0]/tdf2[tdf2['SIS'] < 0].shape[0])
            
            tdf3 = tdf2[(tdf2['SIS'] >= 0) & (tdf2['worst quartile SIR'] == 0)]
            n8.append(tdf3.shape[0])
            n8b.append(100 * tdf3.shape[0]/tdf2[tdf2['SIS'] >= 0].shape[0])
            
            tdf3 = tdf2[(tdf2['SIS'] < 0) & (tdf2['worst quartile SIS'] == 1)]
            n9.append(tdf3.shape[0])
            
            tdf3 = tdf2[(tdf2['SIS'] >= 0) & (tdf2['worst quartile SIS'] == 0)]
            n10.append(tdf3.shape[0])
            
            tdf3 = tdf2[(tdf2['worst quartile SIR'] == 1) & (tdf2['worst quartile SIS'] == 0)]
            n11.append(tdf3.shape[0])
            
            tdf3 = tdf2[(tdf2['worst quartile SIR'] == 0) & (tdf2['worst quartile SIS'] == 1)]
            n12.append(tdf3.shape[0])
            
            tdf3 = tdf2[(tdf2['SIS'] < 0) & (tdf2['worst quartile SIR'] == 1)]
            n13.append(100 * tdf3.shape[0]/tdf2[(tdf2['SIS'] < 0)].shape[0])
            ...
            tdf3 = tdf2[tdf2['SIS'] == 0]
            n17.append(tdf3.shape[0])
            
            n18.append(np.nanmin(tdf2['SIS']))
            n19.append(np.nanmax(tdf2['SIS']))
        ...
        avg = str(np.round(np.nanmean(n6b),3))
        std = str(np.round(np.nanstd(n6b),3))
        out.write('Avg median no. of days for hospitals: ' + str(avg) + ' ± ' + std)
        out.write('\n')
        
        avg = str(np.round(np.nanmean(n5),3))
        std = str(np.round(np.nanstd(n5),3))
        avg1 = str(np.round(np.nanmean(n6c),3))
        std1 = str(np.round(np.nanstd(n6c),3))
        out.write('Avg median no. of days for hospitals with min SIR: ' + avg + ' ± ' + std + ' | pdiff = ' + avg1 + ' ± ' + std1)
        out.write('\n')
        
        avg = str(np.round(np.nanmean(n6),3))
        std = str(np.round(np.nanstd(n6),3))
        avg1 = str(np.round(np.nanmean(n6d),3))
        std1 = str(np.round(np.nanstd(n6d),3))
        out.write('Avg median no. of days for hospitals with min SIS: ' + avg + ' ± ' + std + ' | pdiff = ' + avg1 + ' ± ' + std1)
        out.write('\n')
        out.write('\n')
        ...
        avg = str(np.round(np.nanmean(n17),3))
        std = str(np.round(np.nanstd(n17),3))
        out.write('Avg number of hospitals with SIS of 0: ' + avg + ' ± ' + std)
        out.write('\n')
        
        avg = str(np.round(np.nanmean(n18),3))
        std = str(np.round(np.nanstd(n18),3))
        out.write('Avg min SIS: ' + avg + ' ± ' + std)
        out.write('\n')
        
        avg = str(np.round(np.nanmean(n19),3))
        std = str(np.round(np.nanstd(n19),3))
        out.write('Avg max SIS: ' + avg + ' ± ' + std)
        out.write('\n')
        out.write('\n')
        out.write('\n')
